...
class _FooterState extends State<Footer> {
    ...
    Widget _sectionLeft(TextStyle textStyle) {
        return Column(
            ...
            children: [
                Text('Opening hours', key: const Key('footer_section_left_title'), style: textStyle.copyWith(fontWeight: FontWeight.bold)),
                const SizedBox(height: 6),
                Text('PLEASE NOTE THE UNION SHOP WILL BE CLOSED ALL DAY ON 07/11/2025', style: textStyle, softWrap: true, overflow: TextOverflow.visible),
                const SizedBox(height: 6),
    ...
    Widget _sectionMiddle(TextStyle textStyle) {
        return Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
                Text('Help and Information', key: const Key('footer_section_middle_title'), style: textStyle.copyWith(fontWeight: FontWeight.bold)),
                const SizedBox(height: 6),
                Text('Search', style: textStyle),
                const SizedBox(height: 4),
                Text('Terms and Conditions of Sale Policy', style: textStyle, softWrap: true),
            ],
        );
    }

    Widget _sectionRight(BuildContext context, bool desktop, TextStyle textStyle) {
        ...
        if (desktop) {
            return Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                    Text('Subscribe', key: const Key('footer_section_right_title'), style: textStyle.copyWith(fontWeight: FontWeight.bold)),
                    const SizedBox(height: 6),
    ...
    Widget build(BuildContext context) {
        return LayoutBuilder(builder: (context, constraints) {
            ...
            const TextStyle mobileStyle = TextStyle(fontSize: 14, color: Colors.black87);
            const TextStyle desktopStyle = TextStyle(fontSize: 16, color: Colors.black87);
